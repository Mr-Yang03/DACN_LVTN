...
//             <TableHeader>
//               <TableRow>
//                 <TableHead className="text-center cursor-pointer" style={{ width: "20%" }} onClick={() => toggleSort("full_name")}>Họ tên <ArrowUpDown className="inline w-4 h-4 ml-1" /></TableHead>
//                 <TableHead className="text-center" style={{ width: "20%" }}>Ngày sinh</TableHead>
//                 <TableHead className="text-center cursor-pointer" style={{ width: "20%" }} onClick={() => toggleSort("phone_number")}>SĐT <ArrowUpDown className="inline w-4 h-4 ml-1" /></TableHead>
//                 <TableHead className="text-center" style={{ width: "20%" }}>Số GPLX</TableHead>
//                 <TableHead className="text-center" style={{ width: "20%" }}> </TableHead>
//               </TableRow>
//             </TableHeader>
//             <TableBody>
...
//                   <TableCell className="text-center">{user.license_number || ""}</TableCell>
//                   <TableCell className="text-center space-x-2">
//                     <Button variant="outline" onClick={() => setEditingUser(user)} title="Đặt lại mật khẩu">
//                       <KeyRound size={16} />
//                     </Button>
...
const fetchData = async () => {
  try {
    const res = await getUsers();
    const usersData: User[] = Array.isArray(res.data?.data) ? res.data.data : [];
  ...
  const filteredSortedUsers = users
    .filter((user) => {
      ...
      return name.includes(searchName.toLowerCase()) && phone.includes(searchPhone.toLowerCase());
    })
    .sort((a, b) => {
      if (!sortField) return 0;
      const aVal = a[sortField] || "";
      ...
      <Card className="shadow-md border">
        ...
        <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="text-center cursor-pointer" style={{ width: "20%" }} onClick={() => toggleSort("full_name")}>
                  Họ tên <ArrowUpDown className="inline w-4 h-4 ml-1" />
                </TableHead>
                <TableHead className="text-center" style={{ width: "20%" }}>Ngày sinh</TableHead>
                <TableHead className="text-center cursor-pointer" style={{ width: "20%" }} onClick={() => toggleSort("phone_number")}>
                  SĐT <ArrowUpDown className="inline w-4 h-4 ml-1" />
                </TableHead>
                <TableHead className="text-center" style={{ width: "15%" }}>Số GPLX</TableHead>
                <TableHead className="text-center" style={{ width: "10%" }}>Phản ánh</TableHead>
                <TableHead className="text-center" style={{ width: "15%" }}></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {paginatedUsers.map((user) => (
                <TableRow key={user._id}>
                  ...
                  <TableCell className="text-center">{user.phone_number || ""}</TableCell>
                  <TableCell className="text-center">{user.license_number || ""}</TableCell>
                  <TableCell className="text-center"> {feedbackCounts?.[user.account_id] ?? "Chưa có"}</TableCell>
                  <TableCell className="text-center space-x-2">
                    <Button variant="outline" onClick={() => setEditingUser(user)} title="Đặt lại mật khẩu">
                      <KeyRound size={16} />
                    </Button>
